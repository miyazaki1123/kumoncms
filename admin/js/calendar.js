...
function chgForm(YY,MM,DD){
	...
	this['document'][formNm][monthNm].value = set_MM;
	this['document'][formNm][dateNm].value = set_DD;
...
function viewCalen(MM, set_form, set_calenArea, set_yy, set_mm, set_dd, set_bf, set_af){
...
function createCalen(MM){
	...
	for(i=0; i<6; i++){
		...
		for(j=0; j<7; j++){
			...
			select_yy = this['document'][formNm][yearNm].value;
			select_mm = this['document'][formNm][monthNm].value -1;
			select_dd = this['document'][formNm][dateNm].value;

			if(dateNum == select_dd && theMonth == select_mm && theYear == select_yy){
				calenData += ' class="selctday" ';
			}
